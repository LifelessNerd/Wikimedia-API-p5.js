...
function gotData(data){
  ...
  let orderedFrequencies = orderFrequencies(frequencies);
  
  drawWordCloud(orderedFrequencies);

}

function drawWordCloud(orderedFrequencies){

  var data = [];
  for(var index = 0; index < wordCloudAmount; index++){
    
    let dataObject = new Object();

    dataObject["x"] = orderedFrequencies[index][0];
    dataObject["value"] = orderedFrequencies[index][1]
...
function stringFrequency(rawText){
  ...
  if (!caseSensitive){
    replacedText = replacedText.toLowerCase(); //Makes all words lowercase 
  }
